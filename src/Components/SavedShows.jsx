...
const SavedShows = () => {
  ...
  //to save movir in db
  useEffect(() => {
    onSnapshot(doc(db, "users", `${users?.email}`), (doc) => {
      setMovies(doc.data()?.savedshows);
    });
  }, [users?.email]);

  //to delete movie from db
  const movieRef = doc(db, "users", `${users?.email}`);
      ...
      <div className="relative flex items-center group ">
        ...
        >
          {movies?.map((item, id) => (
            <div
              key={id}
              className="w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block relative cursor-pointer "
            >
              <img
